...
- See Section 19 for current state (session 21 update).

## 18. SYNTHESIS HACKATHON REGISTRATION (session 21 — 2026-03-18)

AlliGo is officially entered in **The Synthesis** hackathon (synthesis.md) — $100K+ prize pool, deadline March 22.

### Credentials (CRITICAL — back up to Railway env vars)
```
SYNTHESIS_API_KEY=sk-synth-212c817abec4986cae34e67f9b4b4fd0edec7af6571dea78
SYNTHESIS_PARTICIPANT_ID=f854c09932ef427596ae92b4a1ac476a
SYNTHESIS_TEAM_ID=4651cfe67b9d42e4a65f2d0639c8bdb6
SYNTHESIS_PROJECT_UUID=b197e78f5d954eceb84581b2dc4cf1c7
```

### On-chain ERC-8004 registration transaction
https://basescan.org/tx/0x456c64630cbb18efb54dd19d4750431ddc8052d844f1fc836322f7f170f0d20c

### Registered agent wallet (auto-generated by Synthesis platform)
`0x6FFa1e00509d8B625c2F061D7dB07893B37199BC`

### Project slug
`alligo-the-credit-bureau-for-ai-agents-acc2`

### Tracks entered (4)
1. **Agents With Receipts — ERC-8004** (Protocol Labs) — $8,004 pool — `3bf41be958da497bbb69f1a150c76af9`
2. **Synthesis Open Track** (Community) — $25,059 pool — `fdb76d08812b43f6a5f454744b66f590`
3. **Agent Services on Base** (Base) — $5,000 pool — `6f0e3d7dcadf4ef080d3f424963caff5`
4. **Ship Something Real with OpenServ** (OpenServ) — $4,500 pool — `9bd8b3fde4d0458698d618daf496d1c7`
...
curl https://synthesis.devfolio.co/projects/b197e78f5d954eceb84581b2dc4cf1c7 \
  -H "Authorization: Bearer sk-synth-212c817abec4986cae34e67f9b4b4fd0edec7af6571dea78"

# Update project (e.g. add video URL)
curl -X PUT https://synthesis.devfolio.co/projects/b197e78f5d954eceb84581b2dc4cf1c7 \
  -H "Authorization: Bearer sk-synth-212c817abec4986cae34e67f9b4b4fd0edec7af6571dea78" \
  -H "Content-Type: application/json" \
  -d '{"videoURL": "https://..."}'

# Publish (after self-custody transfer)
curl -X POST https://synthesis.devfolio.co/projects/b197e78f5d954eceb84581b2dc4cf1c7/publish \
  -H "Authorization: Bearer sk-synth-212c817abec4986cae34e67f9b4b4fd0edec7af6571dea78"
```

---

## 19. PRODUCTION STATE (as of 2026-03-18 session 21)

- **96 claims** tracked, **$4.025B+** total value analyzed
- **38 unique agents** tracked (hero stat corrected from 96 to 38)
- **100%** calibration accuracy (72 tests), **13 predictions** (92% hit rate)
- **60/60** EAS attested, **15 swarm agents** running
- **Landing page**: fully production-ready (all 10 bugs fixed + 3 cleanup issues)
- **Synthesis hackathon**: registered + draft submitted (session 21)
- **Pending Carlos**:
  - ⚡ Add SYNTHESIS_API_KEY to Railway env vars (for wipeout safety)
  - ⚡ Complete self-custody transfer on Synthesis to publish project
  - ⚡ Send ERC-8004 birthday emails manually (drafts in Telegram @alligo_alerts)
  - ⚡ Send Daydreams acquisition tweet
  - EAS ETH top-up (on hold)

---

*Last updated: 2026-03-18 by Zaia (session 21)*
...
*s21: Landing page fully polished (all bugs fixed). Synthesis hackathon registered (ERC-8004 on Base: 0x456c6463...). Project draft submitted to 4 tracks: ERC-8004/Protocol Labs, Open Track, Agent Services on Base, OpenServ. SYNTHESIS_API_KEY saved. Self-custody transfer pending to publish.*
